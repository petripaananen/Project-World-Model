...
async def _execute_agent_pipeline(
    ...
    if mode == "analyze":
        ...
        state.total_input_tokens = config.models.demo_input_tokens
        state.total_output_tokens = config.models.demo_output_tokens
...
async def agent_worker(
    ...
    while True:
        ...
        try:
            ...
            err_msg = str(e)
            print(f"❌ Critical error in agent worker: {err_msg}")
            if event_logger:
                try:
                    await event_logger.log_error(state.run_id, err_msg)
                except Exception:
...
async def run_pipeline(
    ...
    print(
        f"\n🌍 PWM Pipeline Starting "
        f"(run: {state.run_id}, mode: {mode})\n"
    )

    # ── Layer 1: Ingestion ──────────────────────────────────────
    print("📡 Layer 1: Ingesting project data...")
    ...
    print("🔍 Layer 2: Analyzing integration debt...")
